...
const Tabs = ({ children }: { children: React.ReactElement }) => {
  ...
  const tabRefs: React.RefObject<Array<HTMLElement | null>> = useRef([]);
  useEffect(() => {
    if (defaultFocus) {
    ...
    <div className="tab">
        ...
        {tabLinks.map(
          (item: { name: string; children: string }, index: number) => (
            <li
              ...
              // biome-ignore lint/suspicious/noAssignInExpressions: Standard React ref callback pattern
      ...
      {tabLinks.map((item: { name: string; children: string }, i: number) => (
        <div
          ...
          key={i}
          // SECURITY: Content is trusted - sourced from git-controlled markdown files only
